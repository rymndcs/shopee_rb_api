...
module ShopeeRbApi
  # A token pair. Expiries are absolute UTC Times computed from the response and the injected clock.
  # merchant_ids is a Shopee extension (main-account grants).
  Grant = Data.define(:access_token, :refresh_token, :access_token_expires_at, :refresh_token_expires_at,
                      :shop_ids, :merchant_ids, :raw) do
    include Redaction
    ...
    def inspect
      "#<#{self.class.name} access_token=#{Redaction::REDACTED} refresh_token=#{Redaction::REDACTED} " \
        ...
        "merchant_ids=#{merchant_ids.inspect}>"
    end
    alias_method :to_s, :inspect
  end

  # One shop authorized to the app. #locator is the Hash to pass to Client#shop.
  ...
  # A parsed inbound webhook (Shopee push).
