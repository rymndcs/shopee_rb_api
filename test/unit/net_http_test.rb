...
class NetHttpTest < Minitest::Test
  ...
  def test_round_trip
    ...
    received = serve_once(reply) do |port|
      result = ShopeeRbApi::Transport::NetHttp.new.call(method: :post, url: "http://127.0.0.1:#{port}/api/v2/x?a=1",
    ...
    assert_match %r{\APOST /api/v2/x\?a=1 HTTP/1.1}, received
  ...
  def test_connection_refused_is_a_transport_error
    ...
    assert_raises(ShopeeRbApi::TransportError) do
      ShopeeRbApi::Transport::NetHttp.new(open_timeout: 1).call(method: :get, url: "http://127.0.0.1:#{port}/",
                                                                headers: {}, body: nil)
    end
  end
  ...
  def test_unknown_method
    assert_raises(KeyError) do
      ShopeeRbApi::Transport::NetHttp.new.call(method: :patch, url: "http://127.0.0.1:1/", headers: {}, body: nil)
    end
  end
end
